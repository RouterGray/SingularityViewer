...
#endif
		<< (gSavedSettings.getBOOL("VoiceMultiInstance") ? "<MinimumPort>30000</MinimumPort><MaximumPort>50000</MaximumPort>" : LLStringUtil::null)
		<< "<Logging>"
...
bool LLVivoxVoiceClient::startAndLaunchDaemon()
{
    ...
    if (!isGatewayRunning())
    {
        ...
        // Use the Executable dir so voice works in dev checkouts
        std::string exe_path = gDirUtilp->getExecutableDir(); //gDirUtilp->getAppRODataDir();
#if LL_WINDOWS
        gDirUtilp->append(exe_path, "SLVoice.exe");
#elif LL_DARWIN
        gDirUtilp->append(exe_path, "../Resources/SLVoice");
#else
#ifdef LL_STANDALONE
		// Look for the vivox daemon in the executable path list
		// using glib first.
		if (char* voice_path = g_find_program_in_path ("SLVoice"))
		{
...
LLVivoxVoiceClient::participantStatePtr_t LLVivoxVoiceClient::sessionState::addParticipant(const std::string &uri)
{
	...
	if (participantStatePtr_t p = findParticipant(uri))
		return p;
	...
	const std::string& desired_uri = (!mAlternateSIPURI.empty() && (uri == mAlternateSIPURI)) ? mSIPURI : uri;
		...
		// Try to do a reverse transform on the URI to get the GUID back.
		{
			...
			{
				mParticipantList.push_back(participantStatePtr_t(new participantState(desired_uri, id, true)));
			}
			else
			{
				...
				id.generate(uri);
				mParticipantList.push_back(participantStatePtr_t(new participantState(desired_uri, id, false)));
			}
		}

		participantStatePtr_t result = mParticipantList.back();
...
// Singu Note: The only real necessary call to this immediately followed a findParticipant call.
...
{
	participantList::iterator iter = std::find_if(mParticipantList.begin(), mParticipantList.end(), boost::bind(&participantList::value_type::element_type::mURI, _1) == uri);
	if (iter != mParticipantList.end())
	{
		...
		mParticipantsChanged = true;
		LL_DEBUGS("Voice") << "participant \"" << (*iter)->mURI <<  "\" (" << (*iter)->mAvatarID << ") removed." << LL_ENDL;
	}
	else
	{
...
bool LLVivoxVoiceClient::isParticipant(const LLUUID &speaker_id)
{
	...
	return findParticipantByID(speaker_id) != nullptr;
}
...
LLVivoxVoiceClient::participantStatePtr_t LLVivoxVoiceClient::sessionState::findParticipant(const std::string &uri)
{
	...
	if(iter == mParticipantList.end())
	{
		if(!mAlternateSIPURI.empty() && (uri == mAlternateSIPURI))
		{
			...
			iter = std::find_if(mParticipantList.begin(), mParticipantList.end(), boost::bind(&participantList::value_type::element_type::mURI, _1) == mSIPURI);
		}
	}
...
LLVivoxVoiceClient::participantStatePtr_t LLVivoxVoiceClient::sessionState::findParticipantByID(const LLUUID& id)
{
    ...
    participantList::iterator iter = std::find_if(mParticipantList.begin(), mParticipantList.end(), boost::bind(&participantList::value_type::element_type::mAvatarID, _1) == id);
